                            ...
                            "WCGBTS.Slope", "WCGBTS.Hypoxia", "WCGBTS.Santa.Barb.Basin", "WCGBTS.Shelf.Rockfish", "WCGBTS.Video"), verbose = FALSE, optionDigitsAtLeast11 = TRUE, headOnly = FALSE) 
{
    ...
    for (P in projectShort) {
        ...
        if(!is.data.frame(SP) | is.null(ncol(SP)) | noColFlag) {
            ...
            SP <- rename_columns(SP, newname = c("Project", "Trawl_id", "Year", "Vessel", "Pass", "Common_Name", "Scientific_Name", "Tow", "Date", "Depth_m", "Weight_kg", "Length_cm", "Width_cm", "Sex", "Age", "Latitude_dd", "Longitude_dd"))
            if(verbose) { print(SP[1:4,]); cat("\n\n") }
            "  # SP.After <<- SP  "
            
            SP <- SP[, c("Project", "Trawl_id", "Year", "Vessel", "Pass", "Tow", "Date", "Depth_m", "Common_Name", "Scientific_Name", "Weight_kg", "Length_cm", "Width_cm", "Sex", "Age", "Latitude_dd", "Longitude_dd")]
            SP$Project <- P
