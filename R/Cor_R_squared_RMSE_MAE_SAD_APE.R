      ...
      Correlation = cor(Pred, Obs),
        R_squared = cor(Pred, Obs)^2, 
             RMSE = sqrt(mean((Pred - Obs)^2, na.rm = na.rm)), 
			  ...
			  APE = mean(apply(cbind(ObsZ, PredZ), 1, function(x) 100 * sum(abs(na(x, na.rm = na.rm)  - mean(x, na.rm = na.rm))/mean(x, na.rm = na.rm))/length(na(x, na.rm = na.rm)))),
              N = length(na(x, na.rm = na.rm))
    ), digits = digits))
