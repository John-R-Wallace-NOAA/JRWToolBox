        ...
        sweptAreaInHectares = FALSE, rhoConfig. = NULL, numCol =13, Graph.Dev = "tif") 
{
    ...
    JRWToolBox::lib(TeachingDemos, pos=1000)   # Put in back search position because of a conflict with %<=% function in my toolbox.
    ...
    if(is.null(SP.Results.Dpth.)) {
       ...
       SP.Results.Dpth. <- JRWToolBox::plot_maps_JRW(plot_set = 3, MappingDetails=MapDetails_List.[["MappingDetails"]], Report=Report., Sdreport=Opt.$SD, PlotDF=MapDetails_List.[["PlotDF"]], 
                     ...
                     cex=1.8, Legend=MapDetails_List.[["Legend"]], zone=MapDetails_List.[["Zone"]], add = FALSE, mfrow = c(4, 5))
    ...
    # numCol colors 
    SP.Results <- SP.Results.Dpth.[,-(1:2)]  # Removing the Eastings and Northings
    SP.Results[,-(1:2)] <- SP.Results[,-(1:2)] - min(SP.Results[,-(1:2)])  # Now -(1:2) is removing the lat/long from the calculations
