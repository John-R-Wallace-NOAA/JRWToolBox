Spec.code.f <- function(com.name, year = "052002", char = F)
{
	...
	if(nrow(out) == 0) {
		if(length(get.subs(com.name, sep = " ")) == 2) {
			...
			com.name <- paste(casefold.f(tmp[1]), casefold.f(tmp[2], FALSE), sep = " ", collapse = " ")
			out <- Spec.code[grep(paste(com.name, sep = ""), Spec.code$CommonName),  ]
		}
		else out <- Spec.code[grep(paste(casefold.f(com.name), sep = ""), Spec.code$CommonName),  ]
	}
	out2 <- out
	cat("\n")
	...
	if(nrow(out) == 0) {
		if(length(get.subs(com.name, sep = " ")) == 2) {
			tmp <- get.subs(com.name, sep = " ")
			com.name <- paste(casefold.f(tmp[1]), tmp[2], sep = " ", collapse = " ")
			out <- Spec.code[grep(paste(com.name, sep = ""), Spec.code$ScientificName),  ]
		}
		else out <- Spec.code[grep(paste(casefold.f(com.name), sep = ""), Spec.code$ScientificName),  ]
	}
