...
gPlot <- function(data, x.name, y.name, xFunc = NULL, xlab = NULL, ylab = NULL, plotRegionColor = rgb(235, 235, 235, maxColorValue = 255), 
         col = 'black', pch = 1, grid = TRUE, vertLineEachPoint = FALSE, Type = "", verbose = FALSE, ...) {
    ...
    plot(Formula, data = data, xlab = ifelse(is.null(xlab), ifelse(is.null(xFunc), x.name, paste0(xFunc.name, '(', x.name, ')')), xlab), ylab = ifelse(is.null(ylab), y.name, ylab), ...)
