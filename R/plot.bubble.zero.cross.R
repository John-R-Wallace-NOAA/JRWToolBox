plot.bubble.zero.cross <- function (xyzRaw, group = rep("A", nrow(xyzRaw)), Groups = unique(group), Circle.Area.Prop.to.Z = TRUE, maxsize = scale.size * diff(range(xyzRaw[, 2], na.rm = TRUE)), 
    ...
    if (legend) {
       ...
       #   text(Usr[1] + (legLoc[1] + 0.125) * (Usr[2] - Usr[1]), Usr[3] + (legLoc[2] - 0.175) * (Usr[4] - Usr[3]), format(Large, nsmall = legNsmall), adj = 1, cex = 0.9)
        ...
        circle.f(Usr[1] + legLoc[1] * (Usr[2] - Usr[1]), Usr[3] + (legLoc[2] - 0.075) * (Usr[4] - Usr[3]), maxsize * ifelse(Circle.Area.Prop.to.Z, sqrt(Small), Small)/max(xyzSqrt[, 3], na.rm = T), fill.col = col.alpha(legCol, legAlpha))
        text(Usr[1] + (legLoc[1] + 0.125) * (Usr[2] - Usr[1]), Usr[3] + (legLoc[2] - 0.075) * (Usr[4] - Usr[3]), format(Small, nsmall = legNsmall), adj = 1, cex = 0.9)
        
        circle.f(Usr[1] + legLoc[1] * (Usr[2] - Usr[1]), Usr[3] + (legLoc[2] - 0.115) * (Usr[4] - Usr[3]), maxsize * ifelse(Circle.Area.Prop.to.Z, sqrt(Mid), Mid)/max(xyzSqrt[, 3], na.rm = T), fill.col = col.alpha(legCol, legAlpha))
        text(Usr[1] + (legLoc[1] + 0.125) * (Usr[2] - Usr[1]), Usr[3] + (legLoc[2] - 0.115) * (Usr[4] - Usr[3]), format(Mid, nsmall = legNsmall), adj = 1, cex = 0.9)
        
        circle.f(Usr[1] + legLoc[1] * (Usr[2] - Usr[1]), Usr[3] + (legLoc[2] - 0.175) * (Usr[4] - Usr[3]), maxsize * ifelse(Circle.Area.Prop.to.Z, sqrt(Large), Large)/max(xyzSqrt[, 3], na.rm = T), fill.col = col.alpha(legCol, legAlpha))
        text(Usr[1] + (legLoc[1] + 0.125) * (Usr[2] - Usr[1]), Usr[3] + (legLoc[2] - 0.175) * (Usr[4] - Usr[3]), format(Large, nsmall = legNsmall), adj = 1, cex = 0.9)
