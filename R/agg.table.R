agg.table <- function (df, rows = NULL, cols = NULL, grandTotal.col = F, Print = TRUE, digits = 7, 
               sort.cols = 1, sortColNames = FALSE, rows.as.col = F, NA.to.zeros = F) 
{
    ...
    row.len <- length(row)
    col.len <- length(col)
    out <- matrix(NA, nrow = row.len, ncol = col.len, dimnames = list(as.character(row), 
        as.character(col)))
    for (i in 1:row.len) {
        for (j in 1:col.len) {
            try(out[i, j] <- df[df[[1]] == row[i] & df[[2]] == 
                col[j], 3], silent = TRUE)
        }
    }
    out2 <- data.frame(out)
