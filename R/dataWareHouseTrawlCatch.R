...
dataWareHouseTrawlCatch <- function (Species = "Sebastes pinniger", YearRange = c(1000, 5000), projectShort = "Ask", verbose = FALSE, optionDigitsAtLeast11 = TRUE) 
{
    ...
    " "
    Vars <- c("scientific_name", "year", "subsample_count", "subsample_wt_kg", 
    ...
    "  # SP.Before <<- SP  "
    SP <- rename_columns(SP, newname = c("Year", "Vessel", "Tow", 
        "Scientific_Name", "Subsample_count", "Subsample_wt_kg", 
        "Total_sp_numbers", "Total_sp_wt_kg"))
    if (verbose) {
        print(SP[1:4, ])
        cat("\n\n")
    }
    ...
    SP <- SP[, c("Year", "Vessel", "Tow", "Scientific_Name", 
        "Subsample_count", "Subsample_wt_kg", "Total_sp_numbers", 
        "Total_sp_wt_kg")]
    " "
    " # Match SP to all tows to get the zeros "
    " "
    Vars <- c("project", "year", "vessel", "pass", "tow", "date_dim$full_date", 
    ...
    All.Tows <- rename_columns(All.Tows, newname = c("Project", "Trawl_id", 
        "Year", "Pass", "Vessel", "Tow", "Date", "Depth_m", "Longitude_dd", 
        "Latitude_dd", "Area_Swept_ha"))
    if (verbose) {
        print(All.Tows[1:4, ])
        cat("\n\n")
    }
    ...
    All.Tows <- All.Tows[!duplicated(paste(All.Tows$Year, All.Tows$Pass, 
        All.Tows$Vessel, All.Tows$Tow)), c("Project", "Trawl_id", "Year", 
        "Pass", "Vessel", "Tow", "Date", "Depth_m", "Longitude_dd", 
        "Latitude_dd", "Area_Swept_ha")]
    ...
    Out$Date <- chron(format(as.POSIXlt(Out$Date, format = "%Y-%m-%dT%H:%M:%S"), 
