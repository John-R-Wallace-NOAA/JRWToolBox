install_github_save_SHA <- function (repo, ref = "master", ...) 
{
    ...
    currentSHA <- JRWToolBox::SHA(get.subs(repo, "/")[[length(get.subs(repo, "/"))]])
    packageName <- devtools:::remote_package_name(lapply(repo, 
        devtools:::github_remote, username = NULL, ref = ref, 
        subdir = NULL, auth_token = devtools:::github_pat(quiet), 
