...
to download just the lib() function which replaces the 2 step process of install.packages('foo'); library(foo) with just one step; lib(foo). If the foo package is already installed, then lib(foo) will only do library(foo), hence lib(foo) is all that is needed when installing and/or loading a package from CRAN.
